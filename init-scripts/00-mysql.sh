...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202309041411-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  00-mysql.sh --help
# @@Copyright        :  Copyright: (c) 2023 Jason Hempstead, Casjays Developments
# @@Created          :  Monday, Sep 04, 2023 14:11 EDT
# @@File             :  00-mysql.sh
# @@Description      :
...
[ -f "$CONF_DIR/env/mysql.script.sh" ] && . "$CONF_DIR/env/mysql.script.sh" # Generated by my dockermgr script
[ -f "$CONF_DIR/env/mysql.sh" ] && . "$CONF_DIR/env/mysql.sh"               # Overwrite the variabes
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
user_name="${MYSQL_USER_NAME:-$user_name}"
user_pass="${MYSQL_USER_PASS_WORD:-$user_pass}"
root_user_name="${MYSQL_ROOT_USER_NAME:-${root_user_pass:-root}}"
root_user_pass="${MYSQL_ROOT_PASS_WORD:-${root_user_pass:-$(__random_password 32)}}"
DATABASE_CREATE="${DATABASE_CREATE:-${MARIADB_DATABASE:-$MYSQL_DATABASE}}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  if __is_dir_empty "$DATABASE_DIR/mysql" || [ ! -f "$DATABASE_DIR/ibdata1" ]; then
    rm -Rf "$DATABASE_DIR"
    mkdir -p "$DATABASE_DIR" && chown -Rf $SERVICE_USER:$SERVICE_GROUP "$DATABASE_DIR"
...
__post_execute() {
  ...
  (
    sleep 20
    DATABASE_NAME="$DATABASE_CREATE" DB_USER_NAME="$user_name" DB_USER_PASS="$user_pass" ADMIN_USER="$root_user_name" ADMIN_PASS="$root_user_pass" db_init_mysql
